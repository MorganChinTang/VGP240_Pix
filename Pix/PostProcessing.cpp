...
void PostProcessing::DrawPixel(int x, int y)
{
    ...
    case EffectType::Sepia:
    {
        ...
        color.b = X::Math::Dot(colorVec, X::Math::Vector3(0.227f, 0.534f, 0.131f));
    }
    break;
    ...
    case EffectType::Wave:
    {
        ...
        color = mRenderTarget.GetColor(x, y);
        float ratio = static_cast<float> (y% yRippleLength) / static_cast<float>(yRippleLength - 1.0f);
        float circleValues = (ratio * X::Math::kPi);
        float sinValue =std::sin(circleValues);
